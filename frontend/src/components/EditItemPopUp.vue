<template>
  <PopUp @close="emit('close')">
    <div class="space-y-5 p-2">
      ...
      <input id="edit-item-price" v-model="priceRef" type="text" />

      <label for="edit-item-weight">Weight:</label>
      <input id="edit-item-weight" v-model="weightRef" type="text" />
...
const priceRef = ref(props.item.price.toString())
const weightRef = ref(props.item.weight.toString())
...
async function saveItem() {
  ...
  if (isNaN(price) || isNaN(weight)) {
    errorText.value = 'Price and weight must be numbers'
    return
  }

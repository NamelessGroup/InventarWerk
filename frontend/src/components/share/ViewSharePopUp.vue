<template>
  <PopUp @close="emit('close')">
    <div>
      <p class="flex gap-2 items-center">Shared by <DiscordImage :user="creator" class="h-8" /> {{ creator.name }}</p>
      <p v-if="hasWriteAccess">Write access: You can add and remove items and money from this inventory.</p>
      <p v-else>Read access: You can view items and money from this inventory, but not edit it.</p>
      <button class="w-full rounded border border-amber-300 bg-fuchsia-900 p-1 outline-none" @click="leaveInventory()">Leave inventory</button>
    </div>
  </PopUp>
</template>
...
const creator = computed(() => {
  return store().accounts.filter(account => account.uuid === props.inventory.owner)[0] ?? { name: 'Unknown', avatar: null, dm: false, uuid: '' };
})
...
async function leaveInventory() {
  if (hasWriteAccess.value) {
    store().removeWriteShare(props.inventory.uuid, store().uuid);
  } else {

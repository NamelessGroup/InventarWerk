<template>
  <PopUp @close="emit('close')">
    <div class="space-y-2 overflow-auto max-h-full">
      <input v-model="searchFieldContent" type="text" placeholder="Search" class="bg-fuchsia-950 rounded border border-amber-300 p-1 outline-none w-full" />
      <PresetGroup v-for="group in allGroups" :key="group" :group="group" :presets="groupedPresets[group]" />
    </div>
  </PopUp>
</template>
...
const filteredPresets = computed(() => {
  return allPresets.value.filter(p => p.name.toLowerCase().includes(searchFieldContent.value.toLowerCase()))
})
